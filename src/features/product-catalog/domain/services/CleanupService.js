...
export class CleanupService {
  ...
  async analyzeCatalog() {
    ...
    Logger.log(`  Providers: ${providers.length}`);
    ...
    if (duplicates.size > 0) {
      ...
      duplicates.forEach((ids, name) => {
        Logger.log(`  "${name}": ${ids.length} duplicates (IDs: ${ids.join(', ')})`);
      });
    }
  ...
  async removeDuplicateProducts() {
    Logger.log('🧹 Removing duplicate products...');
    ...
    for (const [name, ids] of duplicates.entries()) {
      ...
      Logger.log(`    Keep: ${toKeep.id} (updated: ${toKeep.metadata.updatedAt})`);

      for (const product of toRemove) {
        try {
          await this.#catalogService.deleteProduct(product.id);
          Logger.log(`    ✓ Removed: ${product.id}`);
          removedCount++;
        } catch (error) {
          Logger.error(`    ✗ Failed to remove ${product.id}:`, error.message);
        }
      }
    }
  ...
  async removeDuplicateProviders() {
    Logger.log('🧹 Removing duplicate providers...');
    ...
    for (const [name, ids] of duplicates.entries()) {
      ...
      Logger.log(`    Keep: ${toKeep.id}`);

      for (const provider of toRemove) {
        try {
          await this.#catalogService.deleteProvider(provider.id);
          Logger.log(`    ✓ Removed: ${provider.id}`);
          removedCount++;
        } catch (error) {
          Logger.error(`    ✗ Failed to remove ${provider.id}:`, error.message);
        }
      }
    }
  ...
  async restoreRealEstateCategory() {
    Logger.log('🏠 Restoring realEstate category...');

    try {
      const existing = await this.#catalogService.getCategoryByType('realEstate');
      if (existing) {
        Logger.log('✓ RealEstate category already exists');
    ...
    try {
      ...
      Logger.log('✅ RealEstate category restored');
      return { restored: true };
    } catch (error) {
      Logger.error('❌ Failed to restore realEstate category:', error);
